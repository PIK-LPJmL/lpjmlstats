...
create_map_plots <- function(var_grp_list,
  ...
  create_single_map <- function(
    ...
    band_names_short,
    limits = NULL,
    modification_descr = modification_descr
  ) {
    ...
    for (band in seq_along(band_names)) {
      ...
      for (lpjml_calc in var_grp$under_test) {
        map <- create_single_map(
          ...
          band_names,
          band_names_short,
          limits = var_grp$get_limits(
            type = "under_test", quantiles = m_options$quantiles
          ),
          modification_descr = ""
        )
        plot_list[[map$plot_title]] <- map$plot
      ...
      if (!is.null(var_grp$baseline)) {
        ...
        map <- create_single_map(
          ...
          band_names_short,
          limits = var_grp$get_limits(
            type = "baseline", quantiles = m_options$quantiles
          ),
          modification_descr = ""
        )
        plot_list[[map$plot_title]] <- map$plot
